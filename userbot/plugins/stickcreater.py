...
async def sticklet(event):
    ...
    await event.client.send_file(event.chat_id, image_stream, caption="Created Using ArianaUserBot Userbot", reply_to=event.message.reply_to_msg_id)
    # cleanup
    try:
        os.remove(FONT_FILE)
    except:
...
async def sticklet(event):
    ...
    image_stream.seek(0)
    # finally, reply the sticker
    await event.client.send_file(event.chat_id, image_stream, caption="Created Using ArianaUserBot", reply_to=event.message.reply_to_msg_id)
    # cleanup
    try:
        os.remove(FONT_FILE)
    except:
