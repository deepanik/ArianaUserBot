...
CUSTOM_STICKER_NAME =str(CUSTOM_STICKER_PACK_NAME) if CUSTOM_STICKER_PACK_NAME else "My Boss BinBhai Volume Pack One"
...
async def _(event):
    ...
    #packname = f"BinBhai PACK"
    #packshortname = f"BinBhai_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 728421039:
        packname = f"Ceo's Packk 🎭"
        packshortname = "BinBhai_pack_UB"
    else:
    ...
    file_ext_ns_ion = "ArianaUserBot.png"
    ...
    if is_a_s:
        ...
        if userid == 728421039:
            ...
            packname = f"{user.username}'s {pack}"
            packshortname = f"ArianaUserBot_{userid}" # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    #if userid == 728421039:
                       # packname = f"{user.username}'s {pack}"
                       # packshortname = "Vol._{pack}_BinBhai_ke_locker_me"
                   # else:
                       # packname = f"Vol._{pack}_BinBhai{userid}"
                        #packshortname = f"Vol._{pack}_BinBhai_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
                ...
                await silently_send_message(bot_conv, sticker_emoji)
                await silently_send_message(bot_conv, "/done")
    await event.edit(f"**ArianaUserBot Added That Sticker To My Master {DEFAULTUSER} Pack  SucessFully And Can Be Found** [Here](t.me/addstickers/{packshortname})")
